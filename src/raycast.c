...
void draw3(t_game *g, int x)
{
    ...
    while (++y < SH)
    {
        ...
        {
            if (g->rayc.side == 1 && g->rayc.rayDirY < 0)
                g->img->addr[y * SW + x] = g->text.imgSO->addr[g->text.texHeight * g->text.texY + g->text.texX];
            else if (g->rayc.side == 1 && g->rayc.rayDirY > 0)
                g->img->addr[y * SW + x] = g->text.imgNO->addr[g->text.texHeight * g->text.texY + g->text.texX];
            if (g->rayc.side == 0 && g->rayc.rayDirX < 0)
                g->img->addr[y * SW + x] = g->text.imgWE->addr[g->text.texHeight * g->text.texY + g->text.texX];
            else if (g->rayc.side == 0 && g->rayc.rayDirX > 0)
                g->img->addr[y * SW + x] = g->text.imgEA->addr[g->text.texHeight * g->text.texY + g->text.texX];   
